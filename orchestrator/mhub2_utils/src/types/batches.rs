...
impl BatchTransaction {
    pub fn from_proto(input: mhub2_proto::mhub2::SendToExternal) -> Result<Self, GravityError> {
        if input.fee.is_none() || input.token.is_none() {
        ...
        Ok(BatchTransaction {
            ...
            ethereum_recipient: input.external_recipient.parse()?,
            erc20_token: Erc20Token::from_proto(input.token.unwrap())?,
            erc20_fee: Erc20Token::from_proto(input.fee.unwrap())?,
        })
    }
}
...
impl TransactionBatch {
    ...
    pub fn from_proto(input: mhub2_proto::mhub2::BatchTx) -> Result<Self, GravityError> {
...
impl BatchConfirmResponse {
    pub fn from_proto(
        input: mhub2_proto::mhub2::BatchTxConfirmation,
    ) -> Result<Self, GravityError> {
        Ok(BatchConfirmResponse {
            nonce: input.batch_nonce,
            ethereum_signer: input.external_signer.parse()?,
