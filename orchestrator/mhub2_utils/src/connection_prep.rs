...
/// Returns the three major RPC connections required for Mhub2
...
pub async fn create_rpc_connections(
    ...
    if let Some(grpc_url) = grpc_url {
        ...
        let try_base = Mhub2QueryClient::connect(cosmos_grpc_url.clone()).await;
        match try_base {
            // it worked, lets go!
            Ok(val) => {
                grpc = Some(val);
            ...
            Err(e) => {
                ...
                if grpc_url.to_lowercase().contains("localhost") {
                    ...
                    let ipv6 = Mhub2QueryClient::connect(ipv6_url.clone()).await;
                    let ipv4 = Mhub2QueryClient::connect(ipv4_url.clone()).await;
                    ...
                    let https_on_80 = Mhub2QueryClient::connect(https_on_80_url.clone()).await;
                    let https_on_443 = Mhub2QueryClient::connect(https_on_443_url.clone()).await;
                    warn!(
...
pub async fn check_delegate_addresses(
    ...
    let eth_response = client
        .delegate_keys_by_external_signer(DelegateKeysByExternalSignerRequest {
            external_signer: delegate_eth_address.to_string(),
    ...
    match (eth_response, orchestrator_response) {
        (Ok(e), Ok(o)) => {
            ...
            let req_delegate_eth_address: EthAddress = o.external_signer.parse().unwrap();
