...
func TestAttestationIterator(t *testing.T) {
	...
	att1 := &types.ExternalEventVoteRecord{
		Accepted: true,
		Votes:    []string{},
	}
	dep1 := &types.SendToHubEvent{
		EventNonce:     1,
		ExternalCoinId: TokenContractAddrs[0],
		Amount:         sdk.NewInt(100),
		Sender:         EthAddrs[0].String(),
		CosmosReceiver: AccAddrs[0].String(),
	}
	att2 := &types.ExternalEventVoteRecord{
		Accepted: true,
		Votes:    []string{},
	}
	dep2 := &types.SendToHubEvent{
		EventNonce:     2,
		ExternalCoinId: TokenContractAddrs[0],
		Amount:         sdk.NewInt(100),
		Sender:         EthAddrs[0].String(),
...
func TestDelegateKeys(t *testing.T) {
	...
	for i := range addresses {
		...
		assert.Equal(t, ethAddrs[i].Hex(), res.ExternalAddress)
	}
}
...
func TestStoreEventVoteRecord(t *testing.T) {
	...
	stce := &types.SendToHubEvent{
		EventNonce:     1,
		ExternalCoinId: EthAddrs[0].Hex(),
		Sender:         EthAddrs[0].Hex(),
		CosmosReceiver: AccAddrs[0].String(),
		ExternalHeight: 10,
	...
	evr := &types.ExternalEventVoteRecord{
		Event: stcea,
		Votes: []string{
			ValAddrs[0].String(),
			ValAddrs[1].String(),
	...
	evr2 := &types.ExternalEventVoteRecord{
		Event: cctxea,
		Votes: []string{
			ValAddrs[2].String(),
			ValAddrs[3].String(),
	...
	gk.setExternalEventVoteRecord(ctx, chainId, stce.GetEventNonce(), stce.Hash(), evr)
	gk.setExternalEventVoteRecord(ctx, chainId, cctxe.GetEventNonce(), cctxe.Hash(), evr2)

	stored := gk.GetExternalEventVoteRecord(ctx, chainId, stce.GetEventNonce(), stce.Hash())
	require.NotNil(t, stored)

	stored1 := gk.GetExternalEventVoteRecord(ctx, chainId, cctxe.GetEventNonce(), cctxe.Hash())
	require.NotNil(t, stored1)

	// var storedEvent, storedEvent1 types.ExternalEvent
	...
	mapping := gk.GetExternalEventVoteRecordMapping(ctx, chainId)
...
func TestLastSlashedValsetNonce(t *testing.T) {
	...
	for ; i < 10; i++ {
		...
		_ = k.CreateSignerSetTx(ctx, chainId)
	}

	latestValsetNonce := k.GetLatestSignerSetTxNonce(ctx, chainId)
	assert.Equal(t, uint64(i-1), latestValsetNonce)

	//  lastSlashedValsetNonce should be zero initially.
	lastSlashedValsetNonce := k.GetLastSlashedOutgoingTxBlockHeight(ctx, chainId)
	assert.Equal(t, uint64(0), lastSlashedValsetNonce)
	unslashedValsets := k.GetUnSlashedOutgoingTxs(ctx, chainId, uint64(12))
	assert.Equal(t, 9, len(unslashedValsets))

	// check if last Slashed Valset nonce is set properly or not
	k.SetLastSlashedOutgoingTxBlockHeight(ctx, chainId, uint64(3))
	lastSlashedValsetNonce = k.GetLastSlashedOutgoingTxBlockHeight(ctx, chainId)
	assert.Equal(t, uint64(3), lastSlashedValsetNonce)

	// when maxHeight < lastSlashedValsetNonce, len(unslashedValsets) should be zero
	unslashedValsets = k.GetUnSlashedOutgoingTxs(ctx, chainId, uint64(2))
	assert.Equal(t, 0, len(unslashedValsets))

	// when maxHeight == lastSlashedValsetNonce, len(unslashedValsets) should be zero
	unslashedValsets = k.GetUnSlashedOutgoingTxs(ctx, chainId, uint64(3))
	assert.Equal(t, 0, len(unslashedValsets))

	// when maxHeight > lastSlashedValsetNonce && maxHeight <= latestValsetNonce
	unslashedValsets = k.GetUnSlashedOutgoingTxs(ctx, chainId, uint64(6))
	assert.Equal(t, 2, len(unslashedValsets))

	// when maxHeight > latestValsetNonce
	unslashedValsets = k.GetUnSlashedOutgoingTxs(ctx, chainId, uint64(15))
	...
	t.Run("read after there's one in state", func(t *testing.T) {
		...
		{ // setup
			gk.SetOutgoingTx(ctx, chainId, &types.SignerSetTx{
				Nonce:   gk.incrementLatestSignerSetTxNonce(ctx, chainId),
				Height:  1,
				Signers: nil,
			})
		}
		...
		{ // validate
			got := gk.GetLatestSignerSetTx(env.Context, chainId)
			require.NotNil(t, got)
			require.EqualValues(t, got.Height, gk.GetLatestSignerSetTxNonce(ctx, chainId))
		}
	})
}
...
func TestKeeper_GetEthereumSignatures(t *testing.T) {
	t.Run("read before there's anything in state", func(t *testing.T) {
		...
		storeIndexes := [][]byte{
			...
			types.MakeBatchTxKey(chainId, "", 1), // weird that empty address is okay
			types.MakeContractCallTxKey(chainId, nil, 0),
		}
		for _, storeIndex := range storeIndexes {
			got := gk.GetExternalSignatures(ctx, chainId, storeIndex)
			require.Empty(t, got)
		}
	})
	...
	t.Run("read after there's one signer-set-tx-confirmation in state", func(t *testing.T) {
		...
		{ // setup
			...
			key := gk.SetExternalSignature(ctx, chainId, signerSetTxConfirmation, valAddr)
			require.NotEmpty(t, key)
		}

		{ // validate
			storeIndex := types.MakeSignerSetTxKey(chainId, signerSetNonce)

			{ // getExternalSignature
				got := gk.getExternalSignature(ctx, chainId, storeIndex, valAddr)
				require.Equal(t, []byte("fake-signature"), got)
			}
			{ // GetExternalSignatures
				got := gk.GetExternalSignatures(ctx, chainId, storeIndex)
				require.Len(t, got, 1)
			}
		}
	...
	t.Run("read after there's one batch-tx-confirmation in state", func(t *testing.T) {
		...
		{ // setup
			...
			key := gk.SetExternalSignature(ctx, chainId, batchTxConfirmation, valAddr)
			require.NotEmpty(t, key)
		}

		{ // validate
			storeIndex := types.MakeBatchTxKey(chainId, tokenContract, batchNonce)
	...
	t.Run("read after there's one contract-call-tx-confirmation in state", func(t *testing.T) {
		...
		{ // setup
			...
			key := gk.SetExternalSignature(ctx, chainId, contractCallConfirmation, valAddr)
			require.NotEmpty(t, key)
		}

		{ // validate
			storeIndex := types.MakeContractCallTxKey(chainId, []byte(invalidationScope), invalidationNonce)
...
// TODO review/ensure coverage for:
