...
func TestMsgServer_SubmitEthereumSignature(t *testing.T) {
	...
	msg := &types.MsgSubmitExternalTxConfirmation{
		Confirmation: confirmation,
		Signer:       orcAddr1.String(),
		ChainId:      chainId.String(),
	}

	_, err = msgServer.SubmitTxConfirmation(sdk.WrapSDKContext(ctx), msg)
	require.NoError(t, err)
}

func TestMsgServer_SendToExternal(t *testing.T) {
	ethPrivKey, err := ethCrypto.GenerateKey()
	require.NoError(t, err)
	...
	msg := &types.MsgSendToExternal{
		...
		ExternalRecipient: ethAddr1.String(),
		Amount:            amount,
		BridgeFee:         fee,
		ChainId:           "ethereum",
	}

	_, err = msgServer.SendToExternal(sdk.WrapSDKContext(ctx), msg)
	require.NoError(t, err)
}

func TestMsgServer_CancelSendToExternal(t *testing.T) {
	ethPrivKey, err := ethCrypto.GenerateKey()
	require.NoError(t, err)
	...
	response, err := msgServer.SendToExternal(sdk.WrapSDKContext(ctx), msg)
	require.NoError(t, err)
...
func TestMsgServer_SubmitEthereumEvent(t *testing.T) {
	...
	SendToHubEvent := &types.SendToHubEvent{
		EventNonce:     1,
		ExternalCoinId: "test-token-contract-string",
		Amount:         sdk.NewInt(1000),
		Sender:         ethAddr1.String(),
		CosmosReceiver: orcAddr1.String(),
		ExternalHeight: 200,
	}

	event, err := types.PackEvent(SendToHubEvent)
	require.NoError(t, err)

	msgServer := NewMsgServerImpl(gk)

	msg := &types.MsgSubmitExternalEvent{
		Event:   event,
		Signer:  orcAddr1.String(),
		ChainId: chainId.String(),
	}

	_, err = msgServer.SubmitExternalEvent(sdk.WrapSDKContext(ctx), msg)
	require.NoError(t, err)
}
...
func TestMsgServer_SetDelegateKeys(t *testing.T) {
	...
	msg := &types.MsgDelegateKeys{
		...
		ExternalAddress:     ethAddr1.String(),
		EthSignature:        sig,
	}
