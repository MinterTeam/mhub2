...
func (b *ExternalSigner) ValidateBasic() error {
	if !common.IsHexAddress(b.ExternalAddress) {
...
func (b ExternalSigners) Hash() []byte {
	...
	for _, s := range b {
		out.Write(append(common.HexToAddress(s.ExternalAddress).Bytes(), sdk.Uint64ToBigEndian(s.Power)...))
	}
...
func (b ExternalSigners) PowerDiff(c ExternalSigners) float64 {
	...
	for _, es := range c {
		if val, ok := powers[es.ExternalAddress]; ok {
			powers[es.ExternalAddress] = val - int64(es.Power)
		} else {
			powers[es.ExternalAddress] = -int64(es.Power)
		}
	}
