...
func TestValsetConfirmHash(t *testing.T) {
	...
	members := make(ExternalSigners, len(powers))
	for i := range powers {
		members[i] = &ExternalSigner{
			Power:           powers[i],
			ExternalAddress: ethAddresses[i],
		}
	}

	var mem []*ExternalSigner
...
func TestExternalSigners_PowerDiff(t *testing.T) {
	specs := map[string]struct {
		start ExternalSigners
		diff  ExternalSigners
		exp   float64
	}{
		"no diff": {
			start: ExternalSigners{
...
func TestValsetSort(t *testing.T) {
	specs := map[string]struct {
		...
		"by power desc": {
			src: ExternalSigners{
				{Power: 1, ExternalAddress: gethcommon.BytesToAddress(bytes.Repeat([]byte{byte(3)}, 20)).String()},
				{Power: 2, ExternalAddress: gethcommon.BytesToAddress(bytes.Repeat([]byte{byte(1)}, 20)).String()},
				{Power: 3, ExternalAddress: gethcommon.BytesToAddress(bytes.Repeat([]byte{byte(2)}, 20)).String()},
			},
			exp: ExternalSigners{
				{Power: 3, ExternalAddress: gethcommon.BytesToAddress(bytes.Repeat([]byte{byte(2)}, 20)).String()},
				{Power: 2, ExternalAddress: gethcommon.BytesToAddress(bytes.Repeat([]byte{byte(1)}, 20)).String()},
		...
		"by eth addr on same power": {
			src: ExternalSigners{
				{Power: 1, ExternalAddress: gethcommon.BytesToAddress(bytes.Repeat([]byte{byte(2)}, 20)).String()},
				{Power: 1, ExternalAddress: gethcommon.BytesToAddress(bytes.Repeat([]byte{byte(1)}, 20)).String()},
				{Power: 1, ExternalAddress: gethcommon.BytesToAddress(bytes.Repeat([]byte{byte(3)}, 20)).String()},
			},
			exp: ExternalSigners{
				{Power: 1, ExternalAddress: gethcommon.BytesToAddress(bytes.Repeat([]byte{byte(1)}, 20)).String()},
...
func shuffled(v ExternalSigners) ExternalSigners {
