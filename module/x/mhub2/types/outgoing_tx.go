...
func (sstx *SignerSetTx) GetStoreIndex(chainId ChainID) []byte {
	return MakeSignerSetTxKey(chainId, sstx.Nonce)
}
...
func (cctx *ContractCallTx) GetStoreIndex(chainId ChainID) []byte {
	return MakeContractCallTxKey(chainId, cctx.InvalidationScope.Bytes(), cctx.InvalidationNonce)
}
...
func (b BatchTx) GetCheckpoint(gravityID []byte) []byte {
	...
	for i, tx := range b.Transactions {
		...
		txDestinations[i] = gethcommon.HexToAddress(tx.ExternalRecipient)
	...
	args := []interface{}{
		...
		gethcommon.HexToAddress(b.ExternalTokenId),
...
func (c ContractCallTx) GetCheckpoint(gravityID []byte) []byte {
	...
	for i, coin := range c.Tokens {
		...
		transferTokenContracts[i] = gethcommon.HexToAddress(coin.ExternalTokenId)
	}
	for i, coin := range c.Fees {
		feeAmounts[i] = coin.Amount.BigInt()
		feeTokenContracts[i] = gethcommon.HexToAddress(coin.ExternalTokenId)
	}
